...
function assemble() {
	...
	for (i = 0; i < sloccount; i++) {
		...
		if (/get|print|store|load|add|sub|square|cubic|mod|mul|div|goto|ifpos|ifzero|ifneg|stop/.test(label[i]))
			alert("should '" + label[i] + "' have a space in front of it??");
		opcode[i] = getop(prog[i]);
		if (label[i] == "" && opcode[i] != "" 
		    && !(/get|print|store|load|square|cubic|add|sub|mod|mul|div|goto|ifpos|ifzero|ifneg|stop/.test(opcode[i])))
...
function instr() {
	...
	if (opcode[pc] == "stop") {
      ...
      else if (opcode[pc] == "cubic")
		accumulator = parseFloat(accumulator) * parseFloat(accumulator) * parseFloat(accumulator);
